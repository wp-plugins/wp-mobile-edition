...
define('FDX3_PLUGIN_VERSION', '1.5.1' );
...
define('FDX3_SUPFORUM', 'https://github.com/fabrix/wp-mobile-edition');
...
function fdx_3_admin_notices() {
  if($warning=get_option('fdx_warning')) {
    ...
    print __("ERROR IN SETUP", 'fdx-lang');
    print "</strong></p><p>$warning</p><p><strong>";
    print __('Deactivate and re-activate the plugin', 'fdx-lang').": " .FDX3_PLUGIN_NAME;
    print "</strong></p></div>";
  }
  if($flash=get_option('fdx_flash')) {
    print "<div class='error'><p><strong style='color:#770000'>";
    print __('WARNING', 'fdx-lang');
...
function fdx3_admin_add_page(){
    ...
    add_submenu_page(FDX3_PLUGIN_P1, __('Settings', 'fdx-lang'), __('Settings', 'fdx-lang'), 'manage_options', FDX3_PLUGIN_P1, 'fdx_mobileedition_admin');
...
function fdx_3_readiness_audit() {
  ...
  if (!file_exists($theme_dir)) {
  	$theme_does = __("That directory does not exist.", 'fdx-lang');
  } elseif (!is_writable($theme_dir)) {
  	$theme_does = __("That directory is not writable.", 'fdx-lang');
  } elseif (!is_executable($theme_dir) && DIRECTORY_SEPARATOR=='/') {
  	$theme_does = __("That directory is not executable.", 'fdx-lang');
  }
  if($theme_does!='') {
    $ready = false;
    $why_not[] = sprintf(__('<strong>Not able to install mobile theme files (mTheme)</strong> to %s.', 'fdx-lang'), $theme_dir) . '<p> ' . $theme_does . '</p>';
  }
...
function fdx_3_directory_copy_themes($source_dir, $destination_dir, $benign=true) {
  if(file_exists($destination_dir)) {
	  ...
	  if($dir_does!='') {
      update_option('fdx_warning', sprintf(__('<strong>Not able to install mobile theme files (mTheme)</strong> to ', 'fdx-lang'), $destination_dir) . ' ' . $dir_does);
      return;
    }
  ...
  while($source_file = readdir($dir_handle)) {
    ...
    if (file_exists($destination_child = "$destination_dir/$source_file") && $benign) {
      update_option('fdx_flash',
                    __("Existing mTheme files were found (default mobile theme of plugin)", 'fdx-lang') .
                    "</p><p><strong>" .
                    __("Update of mTheme? (recommended)", 'fdx-lang') .
    ...
    if (file_exists($destination_child) && !is_writable($destination_child)) {
      update_option('fdx_warning', sprintf(__('<strong>Could not install file</strong> to %s.', 'fdx-lang'), $destination_child) . ' ' . __('Please ensure that the web server has write- access to that file.', 'fdx-lang'));
      continue;
    }
